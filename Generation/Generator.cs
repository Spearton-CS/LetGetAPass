...
namespace LetGetAPass.Generation
{
    ...
    public static class Generator
    {
        ...
        public static string Generate(Func<int, int, int> rnd, int len = -1, IReadOnlyList<char>? bounds = null, IReadOnlyList<char>? specials = null, PasswordRequirements reqs = PasswordRequirements.None)
        {
            PasswordRequirements completedReqs = PasswordRequirements.None;
            if (bounds is null)
            {
            ...
            else
            {
                ...
                {
                    if (char.IsUpper(c) && !completedReqs.HasFlag(PasswordRequirements.HaveUppercase))
                        completedReqs |= PasswordRequirements.HaveUppercase;
                    else if (char.IsLower(c) && !completedReqs.HasFlag(PasswordRequirements.HaveLowercase))
                        completedReqs |= PasswordRequirements.HaveLowercase;
                    else if (char.IsDigit(c) && !completedReqs.HasFlag(PasswordRequirements.HaveDigit))
                        completedReqs |= PasswordRequirements.HaveDigit;
                    else if (char.IsLetter(c) && !completedReqs.HasFlag(PasswordRequirements.HaveLetter))
                        completedReqs |= PasswordRequirements.HaveLetter;
                }

                if (!completedReqs.HasFlag(PasswordRequirements.HaveUppercase) && reqs.HasFlag(PasswordRequirements.HaveUppercase))
                    throw new RequirementsIgnoredException("Bounds must have Uppercase char(s). User requirements");
                else if (!completedReqs.HasFlag(PasswordRequirements.HaveLowercase) && reqs.HasFlag(PasswordRequirements.HaveLowercase))
                    throw new RequirementsIgnoredException("Bounds must have Lowercase char(s). User requirements");
                else if (!completedReqs.HasFlag(PasswordRequirements.HaveLowercase) && reqs.HasFlag(PasswordRequirements.HaveDigit))
                    throw new RequirementsIgnoredException("Bounds must have Digit char(s). User requirements");
                else if (!completedReqs.HasFlag(PasswordRequirements.HaveLowercase) && reqs.HasFlag(PasswordRequirements.HaveLetter))
                    throw new RequirementsIgnoredException("Bounds must have Letter char(s). User requirements");
            }
            ...
            if (reqs != PasswordRequirements.None)
            {
                completedReqs = PasswordRequirements.None;
                List<int> ignore = [];
                while (true) //Do requirements check
                {
        ...
        /// <returns></returns>
        public static string GenPseudoRnd(int len = -1, IReadOnlyList<char>? bounds = null, IReadOnlyList<char>? specials = null, PasswordRequirements reqs = PasswordRequirements.None)
            => Generate(Random.Shared.Next, len, bounds, specials, reqs);
        ...
        /// <param name="reqs"></param>
        /// <returns></returns>
        public static string GenPseudoRnd(int seed, int len = -1, IReadOnlyList<char>? bounds = null, IReadOnlyList<char>? specials = null, PasswordRequirements reqs = PasswordRequirements.None)
        {
            Random rnd = new(seed);
            return Generate(rnd.Next, len, bounds, specials, reqs);
        }
        #endregion
        ...
        /// <param name="reqs"></param>
        /// <returns></returns>
        public static string GenCryptoRnd(int len = -1, IReadOnlyList<char>? bounds = null, IReadOnlyList<char>? specials = null, PasswordRequirements reqs = PasswordRequirements.None)
            => Generate(RandomNumberGenerator.GetInt32, len, bounds, specials, reqs);
        #endregion
        ...
        /// <param name="reqs"></param>
        /// <returns></returns>
        public static string GenSha256Rnd(byte[]? seed = null, int len = -1, IReadOnlyList<char>? bounds = null, IReadOnlyList<char>? specials = null, PasswordRequirements reqs = PasswordRequirements.None)
        {
        ...
        /// <param name="reqs"></param>
        /// <returns></returns>
        public static string GenAesRnd(byte[]? seed = null, int len = -1, IReadOnlyList<char>? bounds = null, IReadOnlyList<char>? specials = null, PasswordRequirements reqs = PasswordRequirements.None)
        {
            if (seed is null || seed.Length != 32) //Gen seed
